...
for iChan=1:nClrChans
    ...
    iRow=1;
    iCol=1;
    nRows=size(chanImgPad, 1);
    nCols=size(chanImgPad, 2);
    currChanLBP=nlfilter( chanImgPad, filtDims([1, 2]), @subImgLBP );
    
%     currChanLBP=blockproc(chanImgPad, winStep, @subImgLBP, 'BorderSize', filtDimsR,...
%         'TrimBorder', false, 'PadPartialBlocks', true, 'PadMethod', 'symmetric' );
    ...
    function currChanLBP=subImgLBP(subImg) % BlockStruct
        % find differences between current pixel, and it's neighours
        diffVec=sum(sum( filtR.*repmat(subImg, [1, 1, nNeigh]) )); %BlockStruct.data
        ...
        if isRotInv
            if iChan==1 || isChanWiseRot % go through all posible binary
                ...
                [minLBP, iCircShiftMinLBP(iRow, iCol)]=...
                    sortNeighbours(binaryWord, weigthVec);      % BlockStruct.location
            else % if iChan==1 || isChanWiseRot
                [minLBP, ~]=sortNeighbours( binaryWord, weigthVec,...
                    iCircShiftMinLBP(iRow, iCol) );   % BlockStruct.location
            end % if iChan==1 || isChanWiseRot
        else
            minLBP=weigthVec*binaryWord;
        end % if isRotInv
        currChanLBP=cast( minLBP,  outClass);   % convert to decimal.
    ...
    end % function currChanLBP=subImgLBP(subImg)

end % function LBP= pixelwiseLBP(inImg, varargin) % isRotInv, isChanWiseRot, filtR
